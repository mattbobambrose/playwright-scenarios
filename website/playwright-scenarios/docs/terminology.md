...
| **Source partition** | The subdirectory under `<scenario_dir>` that records *which command produced* a scenario. Three partitions: `<scenario_dir>/crawl/` (from `/crawl-site`), `<scenario_dir>/record/` (from `/record-scenario`), `<scenario_dir>/convert/` (from `/doc-to-scenarios`). Generated tests mirror this partition under `<test_dir>/<command>/<scenario-name>/<ClassName>.kt`. The scenario in its partition is the canonical artifact; users hand-edit or delete in place if they want, then run `/review-scenario`. |
